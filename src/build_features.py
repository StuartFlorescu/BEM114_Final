...
def build_player_features(df):
    ...
    for window in [5, 10, 20]:
        df[f"player_pts_l{window}"] = g["PTS"].transform(lambda s: shifted_rolling_mean(s, window))
    ...
    df["player_pts_per_min_l10"] = df["player_pts_l10"] / df["player_min_l10"]
...
def main():
    ...
    preview_cols = [
        "GAME_DATE", "PLAYER_NAME", "MATCHUP", "MIN", "PTS", "FTA", "PFD",
        "player_pts_l10", "player_pts_l20", "player_pts_per_min_l10",
        "player_fta_l10", "player_pfd_l10", "player_min_l10",
        "opp_pf_l10", "opp_fta_allowed_l10", "simple_matchup_risk"
    ]
    print(modeling[preview_cols].head(20))
